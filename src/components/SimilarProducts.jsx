...
const SimilarProducts = () => {
  ...
  return (
    <div className="font-[sans-serif] py-4 mx-auto lg:max-w-6xl max-w-lg md:max-w-full">
      <h2 className="text-4xl text-center  font-extrabold text-gray-800 mb-9">Similar Products</h2>
          ...
          <div key={product.id} className="bg-gray-200 rounded-xl cursor-pointer hover:scale-[1.03] transition-all relative overflow-hidden">
            <NavLink to="/products" className="">
              ...
              <div className="w-full h-[250px] overflow-hidden mx-auto">
                <img src={product.imgSrc} alt={product.name} className=" w-full object-cover" />
              </div>
            </NavLink>
            <div className="text-center bg-gray-100 p-6">
              <h3 className="text-lg font-bold text-gray-800">{product.name}</h3>
              <h4 className="text-lg text-gray-800 font-bold mt-6">${product.price} <strike className="text-gray-400 ml-2 font-medium">${product.originalPrice}</strike></h4>
              <button
                type="button"
